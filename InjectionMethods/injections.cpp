...
void injectDLL(_In_ DWORD pid, _In_ LPCSTR dllPath) {
    ...
    if (hProcess = ::OpenProcess(dwDesiredAccess, false, pid)) {
        ...
        if (ptrVirtAlloc = ::VirtualAllocEx(hProcess, NULL, sizeof(dllPath), MEM_COMMIT, PAGE_READWRITE)) {
            ...
            if (HANDLE hThread = ::CreateRemoteThread(hProcess, NULL, 0, (LPTHREAD_START_ROUTINE)::GetProcAddress(::GetModuleHandle(L"Kernel32.dll"), "LoadLibrary"), ptrVirtAlloc, 0, &tid)) {
